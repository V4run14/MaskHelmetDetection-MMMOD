...
def detect(save_img=False):
    osource = 'data/images'
    oweights = 'runs/train/yolov5s_results3/weights/best.pt'
    weights2 = 'runs/train/yolov5s_results2/weights/best.pt'
    occtv = 0
    oimg_size = 416
    oconf_thres = 0.7
    oiou_thres = 0.45
    ...
    if classify:
        ...
        modelc2 = load_classifier(name='resnet101', n=3)  # initialize
        modelc.load_state_dict(torch.load('weights/resnet101.pt', map_location=device)['model']).to(device).eval()
        modelc2.load_state_dict(torch.load('weights/resnet101.pt', map_location=device)['model']).to(device).eval()
...
####################################################################################
                ...
                # img2 = letterbox(im0s2, 416, stride=stride)[0]
                ...
                # Apply Classifier
                if classify:
                    pred2 = apply_classifier(pred2, modelc2, img2, im0s2)

                # Process detections
                for i, det in enumerate(pred2):  # detections per image
                    if webcam:  # batch_size >= 1
                        s, im02, frame = '%g: ' % i, im0s2[i].copy(), count
                    else:
                        s, im02, frame = '', im0s2, count
                    ...
                    if len(det):
                        # Rescale boxes from img_size to im0 size
                        det[:, :4] = scale_coords(img2.shape[2:], det[:, :4], im02.shape).round()
                        ...
                        for *xyxy, conf, cls in reversed(det):
                            if save_img:  # Add bbox to image
                                ...
                                plot_one_box(xyxy, im02, label=label, color=colors2[int(cls)], line_thickness=1)

                    # Print time (inference + NMS)
                    print(f'{s}Done. ({t22 - t12:.3f}s)')
                ...
                fps = str(fps)
                cv2.putText(im02, "FPS: {}".format(fps),
                            (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
                ret2, buffer2 = cv2.imencode('.jpg', im02)
